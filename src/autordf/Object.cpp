...
Object Object::cloneRecursiveStopAtIRI(const std::vector<Uri>& doNotClone, const Uri& newIri) const {
    Object res(newIri, this->_rdfTypeIRI);
    copyPropertiesInternal(doNotClone, res);
    copyReifiedProperties(doNotClone, res);
    return res;
}

void Object::copyReifiedProperties(const std::vector<Uri>& doNotClone, Object& to) const {
    ...
    for (const Node & node : reifiedNodes) {
        ...
        bool hasDoNotClone = false;
        for (const Uri &uri : doNotClone) {
            if (reifiedStatement.hasProperty(uri)) {
                hasDoNotClone = true;
                break;
            }
        }
        ...
        switch (object->type()) {
            ...
            case NodeType::RESOURCE:
                ...
                break;
            case NodeType::BLANK:
                reifiedObject = to.reifyObject(predicate->value(), Object(object->asResource()).cloneRecursiveStopAtIRI(doNotClone));
                break;
            default:
...
void Object::copyPropertiesInternal(const std::vector<Uri>& doNotClone, Object& to) const {
    for (const Uri& uri : doNotClone) {
        if (_r.hasProperty(uri)) {
            return;
        }
    }
    std::shared_ptr<std::list<Property>> properties = _r.getPropertyValues();
    // All of these are handled specially - do not copy
    static const std::vector<std::string> forbiddenProperties({RDF_OBJECT, RDF_SUBJECT, RDF_PREDICATE, RDF_SUBJECT});
    for (const Property& property : *properties) {
        ...
        if (property.isLiteral()) {
            ...
            to.addObject(property.iri(), Object(property.asResource()).cloneRecursiveStopAtIRI(doNotClone), false);
        } else {
